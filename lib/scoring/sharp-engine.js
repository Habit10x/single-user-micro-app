...
function normalizeResult(r) {
  ...
  if (r.scoreConfidence  === undefined) r.scoreConfidence  = "MEDIUM"
  if (r.pointFirst       === undefined) r.pointFirst       = null
  if (r.dimensionScores  === undefined) r.dimensionScores  = {}
  if (r.whatWorked       === undefined) r.whatWorked       = []
  if (r.impacts          === undefined) r.impacts          = []
  if (r.oneLiner === undefined && r.summary === undefined) r.oneLiner = ""
  return r
}

export async function sharpEngine(userResponse, context = {}, customBasePrompt = null, customOutputSchema = null, label = "?") {
  // ── MOCK MODE ──────────────────────────────────────────────────────────────
  ...
  const t1 = Date.now()
  console.log(`[sharp-engine] ${label} | prompt: ${t1 - t0}ms | sending to Azure...`)

  const ac  = new AbortController()
  const ato = setTimeout(() => ac.abort(), 25000)
  let res
  try {
  ...
  // Log timing + tokens — skip if input=0 (Azure soft rate-limit refusal, model never ran)
  if (data.usage && data.usage.input_tokens > 0) {
    ...
    const apiMs      = t2 - t1
    const genToks    = reasoning + u.output_tokens
    const estReasoningMs = genToks > 0 ? Math.round((reasoning  / genToks) * apiMs) : 0
    const estOutputMs    = genToks > 0 ? Math.round((u.output_tokens / genToks) * apiMs) : 0
    console.log(`[sharp-engine] ${label} | API: ${apiMs}ms | input:${u.input_tokens} (cached:${cached}) reasoning:${reasoning} output:${u.output_tokens} total:${u.total_tokens}`)
    console.log(`  └─ est. reasoning: ~${estReasoningMs}ms | output: ~${estOutputMs}ms | other: ~${apiMs - estReasoningMs - estOutputMs}ms`)
  }
  ...
  if (!jsonStr.startsWith("{")) {
    ...
    console.log(`[sharp-engine] ${label} | retry | sending to Azure...`)

    const rac  = new AbortController()
    const rato = setTimeout(() => rac.abort(), 25000)
    let retry
    try {
    ...
    const tRetry1    = Date.now()

    if (retryData.usage && retryData.usage.input_tokens > 0) {
      const u         = retryData.usage
      const reasoning = u.output_tokens_details?.reasoning_tokens ?? 0
      const apiMs     = tRetry1 - tRetry0
      const genToks   = reasoning + u.output_tokens
      const estReasoningMs = genToks > 0 ? Math.round((reasoning / genToks) * apiMs) : 0
      const estOutputMs    = genToks > 0 ? Math.round((u.output_tokens / genToks) * apiMs) : 0
      console.log(`[sharp-engine] ${label} | retry API: ${apiMs}ms | input:${u.input_tokens} reasoning:${reasoning} output:${u.output_tokens} total:${u.total_tokens}`)
      console.log(`  └─ est. reasoning: ~${estReasoningMs}ms | output: ~${estOutputMs}ms | other: ~${apiMs - estReasoningMs - estOutputMs}ms`)
    }
    ...
    return normalizeResult(JSON.parse(retryJson))
  }
...
Full response shape (example for 2 scenarios):
{
  ...
  console.log(`[sharp-engine-batched] ${entries.length} scenarios | sending to Azure...`)

  const bac  = new AbortController()
  const bato = setTimeout(() => bac.abort(), 55000) // batched needs more time (N scenarios)
  let res
  try {
    res = await fetch(endpoint, {
      method: "POST",
  ...
  if (data.usage && data.usage.input_tokens > 0) {
    ...
    const cached    = u.input_tokens_details?.cached_tokens ?? u.prompt_tokens_details?.cached_tokens ?? 0
    const reasoning = u.output_tokens_details?.reasoning_tokens ?? 0
    const apiMs     = tBatch1 - tBatch0
    const genToks   = reasoning + u.output_tokens
    const estReasoningMs = genToks > 0 ? Math.round((reasoning / genToks) * apiMs) : 0
    const estOutputMs    = genToks > 0 ? Math.round((u.output_tokens / genToks) * apiMs) : 0
    console.log(`[sharp-engine-batched] API: ${apiMs}ms | input:${u.input_tokens} (cached:${cached}) reasoning:${reasoning} output:${u.output_tokens} total:${u.total_tokens}`)
    console.log(`  └─ est. reasoning: ~${estReasoningMs}ms | output: ~${estOutputMs}ms | other: ~${apiMs - estReasoningMs - estOutputMs}ms`)
  }
  ...
  return Object.fromEntries(Object.entries(parsed).map(([pos, r]) => [pos, normalizeResult(r)]))
}
