...
export async function sharpEngine(userResponse, context = {}, customBasePrompt = null) {
  // ── MOCK MODE ──────────────────────────────────────────────────────────────
  ...
  const systemPrompt = (customBasePrompt || BASE_SYSTEM_PROMPT) + buildScenarioInjection(context) + OUTPUT_FORMAT
  ...
  if (data.usage && data.usage.input_tokens > 0) {
    const u = data.usage;
    console.log(`[sharp-engine] tokens — input:${u.input_tokens} reasoning:${u.output_tokens_details?.reasoning_tokens ?? "?"} output:${u.output_tokens} total:${u.total_tokens}`);
  }
